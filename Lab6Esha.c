...
int divider = 10;
float rightWeight =  1/((4096 - rightClose)*divider);
float leftWeight =  1/((4096 - leftClose)*divider);
float frontWeight =  1/((4096 - frontClose)*divider);
...
int RICorrection = 0; //how far Right Wheel is from the OKRangeMin
int LICorrection = 0; //how far Right Wheel is from the OKRangeMin

//int frontWeightCoefOriginal = 2;
//int leftWeightCoefOriginal = 10;
//int rightWeightCoefOriginal = 10;
//int frontWeightCoef = frontWeightCoefOriginal;
...
//        }
        ...
        FPWMError = IR1Val - frontClose;
        RICorrection = RICorrection + KI* RPWMError;
        LICorrection = LICorrection + KI* LPWMError;
        //front too close  
        if (IR1Val > frontClose){
            red_counter++;
            LEDS = RED;
            //decides based on the two IR 
            if(IR2Val > IR3Val){ //Turn left
                rightMotorSpeed = highSpeedFixed;
                leftMotorSpeed = highSpeedFixed - highSpeedFixed*leftWeight*LPWMError +LICorrection; 
                motorMovement(LEFTMOTOR, STOP, FORWARD,leftMotorSpeed);
                motorMovement(RIGHTMOTOR, MOVE, FORWARD,rightMotorSpeed);
            }
            else{ //Turn right
                leftMotorSpeed = highSpeedFixed;
                rightMotorSpeed = highSpeedFixed - highSpeedFixed*rightWeight*RPWMError +RICorrection; 
                motorMovement(LEFTMOTOR, STOP, FORWARD,leftMotorSpeed);
                motorMovement(RIGHTMOTOR, MOVE, FORWARD,rightMotorSpeed);
            }
        }
        //right wheel close 
        else if(IR2Val > rightClose && IR2Val > IR3Val){
            //            if (red_counter > 0) {
            //                red_counter--;
            //                rightWeightCoef = rightWeightCoefOriginal/2;
            //            }else{
            //                rightWeightCoef = rightWeightCoefOriginal;
            //            }
            LEDS = GREEN;
            leftMotorSpeed = highSpeedFixed;
            rightMotorSpeed = highSpeedFixed - highSpeedFixed*rightWeight*RPWMError +RICorrection; 
        ...
        //left too close 
        else if(IR3Val > leftClose && IR3Val>IR2Val){ //IR0 is on Right side and IR1 in front
            //            if (red_counter > 0) {
            //                red_counter--;
            ////                leftWeightCoef = leftWeightCoefOriginal/2;
            //            }else{
            //                leftWeightCoef = leftWeightCoefOriginal;
            //            }
            LEDS = BLUE; 
