...
#define PWM_ENABLE_PWM1EN       0x0000000C  // PWM1 Output Enable
...
#define PWM_1_GENB_ACTCMPBD_ZERO                                              \
                                0x00000C00  // Set the output signal to 1
#define PWM_1_GENB_ACTCMPBU_ONE 0x00000200  // Set the output signal to 0

#define PWM_1_GENA_ACTCMPAD_ZERO                                              \
                                0x000000C0  // Set the output signal to 1
#define PWM_1_GENA_ACTCMPAU_ONE 0x00000020  // Set the output signal to 0
...
void PWM0Dual_Init(uint16_t period){
  ...
  PWM1_1_GENB_R = (PWM_1_GENB_ACTCMPBD_ZERO|PWM_1_GENB_ACTCMPBU_ONE);
  PWM1_1_LOAD_R = (period - 1)/2;       // 5) count from zero to this number and back to zero in (period - 1) cycles
  PWM1_1_CMPA_R = (period - 1)/2;       // 6) count value when PWM1/PB7 toggles
  PWM1_1_CMPB_R = (period - 1)/128;       // 6) count value when PWM1/PB7 toggles
...
void PWM0Dual_Period(uint16_t period){
    PWM1_1_CMPA_R = (period - 1)/2;  //PA6  
    PWM1_1_CMPB_R = (period - 1)/128; //PA7, smaller value
    //PWM1_1_CMPB_R = 0; //PA7
}
