...
// the Launchpad is wired.  Weak internal pull-up
...
void Board_Init(void){            
  ...
  GPIO_PORTF_CR_R |= SW1;  // 2b) enable commit for PF0
  GPIO_PORTF_CR_R |= SW2;  // 2b) enable commit for PF4 

  GPIO_PORTF_AMSEL_R &= ~(SW1); //3 disable analog functionality for PF0 
  GPIO_PORTF_AMSEL_R &= ~(SW2); //disable analog functionality for PF4 
                                 
  
  GPIO_PORTF_PCTL_R = (GPIO_PORTF_PCTL_R&0xFFF0FFF0)+0x00000000;
  
  GPIO_PORTF_DIR_R &= ~(SW1);// 5) make PF0 input (built-in buttons)
  GPIO_PORTF_DIR_R &= ~(SW2);// 5) make PF4 input (built-in buttons)
  
  
 
  GPIO_PORTF_AFSEL_R &= ~(SW1);  // 6) disable alt funct on PF0 
  GPIO_PORTF_AFSEL_R &= ~(SW2);  // 6) disable alt funct on PF4 
  
  
  //  delay = SYSCTL_RCGC2_R;        // put a delay here if you are seeing erroneous NMI
  GPIO_PORTF_PUR_R |= (SW1); // enable weak pull-up on PF0  
  GPIO_PORTF_PUR_R |= (SW2); // enable weak pull-up on PF4  
  
  GPIO_PORTF_DEN_R |= (SW1); // 7) enable digital I/O on PF0 
  GPIO_PORTF_DEN_R |= (SW2); // 7) enable digital I/O on PF4 
}
*/
